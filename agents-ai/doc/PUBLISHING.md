...
Low-level reference for how AgentsAI packages are prepared and published to npm.
...
AgentsAI no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `agentsai`, imports code from workspace packages such as:
...
### Manual first publish for `@agentsai/ui`
...
npm view @agentsai/ui version

# make sure the dist payload is fresh
pnpm --filter @agentsai/ui build
...
- If `npm view @agentsai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@agentsai/ui`, the `agentsai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
AgentsAI uses calendar versions:
...
- `agentsai@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx agentsai@canary onboard
```
