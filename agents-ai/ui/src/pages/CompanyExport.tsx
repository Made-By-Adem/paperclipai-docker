...
/**
 * Filter .Agents.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterAgentsYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://Agents.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm Agentsai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Agents](https://Agents.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Agents](https://Agents.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .Agents.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .Agents.yaml
    const yamlPath = exportData.AgentsExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterAgentsYaml(exportData.files[yamlPath], checkedFiles);
    }
