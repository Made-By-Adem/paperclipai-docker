...
summary: What adapters are and how they connect agents to AgentsAI
---

Adapters are the bridge between AgentsAI's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, AgentsAI:
...
| Hermes Local | `hermes_local` | Runs Hermes CLI locally (`hermes-agentsai-adapter`) |
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-agentsai-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to AgentsAI's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `agentsai run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the AgentsAI web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
