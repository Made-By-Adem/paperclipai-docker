/**
 * esbuild configuration for building the agentsai CLI for npm.
 *
 * Bundles all workspace packages (@agentsai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @agentsai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@agentsai/")) {
      externals.add(name);
    }
  }
