...
`PAP-447` asks how AgentsAI should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same AgentsAI instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into AgentsAI:

- some operators use AgentsAI to manage AgentsAI and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as AgentsAI
- Claude and Codex expose different built-in affordances, so AgentsAI should not overfit to one tool

## Core Product Decision

AgentsAI should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by AgentsAI
...
- AgentsAI should not assume that every adapter can see or use a host filesystem path directly
...
If AgentsAI makes worktrees agent-first, it will blur:
...
- AgentsAI core resolves and records execution workspace state
...
That gives AgentsAI room to support:
...
- AgentsAI handles workspace runtime services under the hood using project/workspace policy
...
Once AgentsAI is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So AgentsAI should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that AgentsAI cannot infer
...
- when issue moves to done, should AgentsAI auto-commit?
- should it auto-push?
...
AgentsAI already has the right foundation for a project-first model.
...
For server-spawned local adapters, AgentsAI already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that AgentsAI should mirror directly

Implication:

- for `codex_local`, AgentsAI should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but AgentsAI should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, AgentsAI should separate:
...
AgentsAI should be able to express intentions such as:
...
These are not specific to AgentsAI. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".agentsai/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a AgentsAI-specific `devServer` flag.
...
AgentsAI should distinguish between:
...
AgentsAI should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, AgentsAI should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to AgentsAI
...
For runtime services specifically, AgentsAI should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as AgentsAI-only dev-server flags
...
3. Inject resolved execution workspace details into `context.agentsaiWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / AgentsAI instance can be reused through env/config injection when appropriate
...
- AgentsAI can decide whether to reuse or start a fresh service deterministically
...
3. Test against AgentsAI-on-AgentsAI workflows first.
...
7. Workspace runtime services are modeled generically, not as AgentsAI-specific dev-server toggles.
