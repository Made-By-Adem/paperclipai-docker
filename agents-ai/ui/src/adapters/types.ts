...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@agentsai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@agentsai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@agentsai/adapter-utils").TranscriptEntry[];
