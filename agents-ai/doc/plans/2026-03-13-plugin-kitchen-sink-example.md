...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented AgentsAI plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside AgentsAI.
...
- package: `@agentsai/plugin-kitchen-sink-example`
- plugin id: `agentsai.kitchen-sink-example` or `agentsai-kitchen-sink-example`

Recommendation: use `agentsai-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core AgentsAI event such as `issue.created`
...
Recommendation: default to a AgentsAI-local endpoint or a stable public echo endpoint to avoid flaky docs.
