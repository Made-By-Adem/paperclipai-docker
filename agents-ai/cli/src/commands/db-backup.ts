...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@agentsai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://Agents:Agents@127.0.0.1:${port}/Agents`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" Agents db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAgentsInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "Agents";
