# AgentsAI V1 Implementation Spec
...
AgentsAI V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.agentsai/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.agentsai/instances/default/data/storage` (`local_disk`)
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before AgentsAI runtime-owned keys are injected
...
- when AgentsAI cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
- if unset, auto-use embedded PostgreSQL under `~/.agentsai/instances/default/db`
...
- `.agentsai.yaml` sidecar for AgentsAI-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.agentsai.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and AgentsAI routine fidelity in `.agentsai.yaml`
- AgentsAI imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.agentsai.yaml`
