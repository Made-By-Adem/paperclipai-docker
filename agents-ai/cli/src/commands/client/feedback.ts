...
import type { Company, FeedbackTrace, FeedbackTraceBundle } from "@agentsai/shared";
import {
...
export async function resolveFeedbackCompanyId(
  ...
  if (!companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set Agents_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return companyId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Agents Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Agents Feedback Export")));
