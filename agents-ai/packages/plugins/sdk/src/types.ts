/**
 * Core types for the Agents plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@agentsai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @agentsai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Agents object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Agents UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Agents domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Agents domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Agents secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Agents secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@agentsai/plugin-sdk/ui`
 */
