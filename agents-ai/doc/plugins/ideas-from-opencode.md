...
AgentsAI V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a AgentsAI-shaped design.

Assumption for this document: AgentsAI is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class AgentsAI objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into AgentsAI.

The main conclusion is:

- AgentsAI should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- AgentsAI should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- AgentsAI should use multiple extension classes instead of one generic plugin bag:
...
If AgentsAI does this well, the examples you listed become straightforward:
...
Relevant AgentsAI files reviewed for current extension seams:
...
It is too dangerous for AgentsAI core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for AgentsAI — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
AgentsAI should absolutely do this.
...
AgentsAI should copy this discipline.
...
AgentsAI should adopt this style for plugin actions, automations, and UI schemas.
...
AgentsAI can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for AgentsAI too.

## What AgentsAI Should Not Copy Directly
...
AgentsAI is a control plane for an operator-managed instance with company objects.
...
AgentsAI is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

AgentsAI should not auto-load arbitrary code from a workspace repo like `.agentsai/plugins` or project directories.
...
For AgentsAI, equivalent hooks into:
...
AgentsAI should not allow plugins to silently replace:
...
For AgentsAI it would be risky because it combines:
...
AgentsAI should require an explicit operator install step.

## Why AgentsAI Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | AgentsAI |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted AgentsAI instance |
...
That means AgentsAI should borrow the good ideas from `opencode` but use a stricter architecture.

## AgentsAI Already Has Useful Pre-Plugin Seams

AgentsAI has several extension-like seams already:
...
This is good news.
AgentsAI does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended AgentsAI Plugin Model
...
AgentsAI already has this pattern implicitly:
...
- `plugin` means integration code that talks to AgentsAI through a typed plugin protocol and capability model
...
- invoke explicit AgentsAI actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@agentsai/plugin-sdk/ui`) with design tokens and common components.
...
AgentsAI should treat plugin installation as a global instance-level action.

Examples:

- install `@agentsai/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over AgentsAI objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

AgentsAI already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for AgentsAI too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@agentsai/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@agentsai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running AgentsAI instance.

A `create-agentsai-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how AgentsAI achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the AgentsAI server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. AgentsAI's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@agentsai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@agentsai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For AgentsAI

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@agentsai/plugin-sdk";

export default definePlugin({
  id: "@agentsai/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit AgentsAI entities
    });
    ...
    ctx.events.on("plugin.@agentsai/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@agentsai/plugin-sdk/ui";
...
Any AgentsAI plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific AgentsAI objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@agentsai/plugin-linear`)
...
- reuse core tables when the data is clearly part of AgentsAI itself
...
If a concept is becoming part of AgentsAI's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of AgentsAI itself
...
### 3. Scope plugin data to AgentsAI objects before adding custom schemas

A lot of plugin data naturally hangs off existing AgentsAI objects:
...
If AgentsAI eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@agentsai/plugin-workspace-files`
...
Package idea: `@agentsai/plugin-terminal`
...
Package idea: `@agentsai/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving AgentsAI.
- Board reviews the diff after a run without leaving AgentsAI.
...
- optional `events.emit` (e.g. `plugin.@agentsai/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@agentsai/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@agentsai/plugin-linear`

This plugin syncs AgentsAI work with Linear. It is useful for:

- importing backlog from Linear
- linking AgentsAI issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between AgentsAI and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into AgentsAI.
- AgentsAI issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@agentsai/plugin-github-issues`

This plugin syncs AgentsAI issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to AgentsAI issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with AgentsAI task governance
...
- Plugin settings:
  ...
  - whether new AgentsAI issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active AgentsAI issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into AgentsAI.
- GitHub webhooks update status/comment state in AgentsAI.
- A PR is linked back to the AgentsAI issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@agentsai/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@agentsai/plugin-grafana`

This plugin surfaces external metrics and dashboards inside AgentsAI. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating AgentsAI issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full AgentsAI plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the AgentsAI dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a AgentsAI issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of AgentsAI core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@agentsai/plugin-runtime-processes`
...
Package idea: `@agentsai/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into AgentsAI. It is useful for:
...
- Failed payment spikes or churn events can generate AgentsAI issues for follow-up.
...
- Stripe data should stay additive to AgentsAI core
...
- plugin UI bundle loading, host bridge, `@agentsai/plugin-sdk/ui`
...
- `@agentsai/plugin-test-harness` and `create-agentsai-plugin` starter template
...
AgentsAI should not implement "an OpenCode-style generic in-process hook system."
AgentsAI should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In AgentsAI
...
8. Ship `@agentsai/plugin-test-harness` and `create-agentsai-plugin` starter template.
