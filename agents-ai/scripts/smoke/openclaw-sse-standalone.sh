...
OPENCLAW_USER="${OPENCLAW_USER:-agentsai-smoke}"
...
Run your AgentsAI heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  '{
    ...
    metadata: {
      ...
      agentsai_session_key: ("agentsai:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: agentsai:run:${PAPERCLIP_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
