...
describe("agent instructions service", () => {
  const originalAgentsHome = process.env.PAPERCLIP_HOME;
  const originalAgentsInstanceId = process.env.PAPERCLIP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalAgentsHome === undefined) delete process.env.PAPERCLIP_HOME;
    else process.env.PAPERCLIP_HOME = originalAgentsHome;
    if (originalAgentsInstanceId === undefined) delete process.env.PAPERCLIP_INSTANCE_ID;
    else process.env.PAPERCLIP_INSTANCE_ID = originalAgentsInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-home-");
    const externalRoot = await makeTempDir("Agents-agent-instructions-external-");
    cleanupDirs.add(AgentsHome);
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("Agents-agent-instructions-new-external-");
    cleanupDirs.add(AgentsHome);
    cleanupDirs.add(externalRoot);
    process.env.PAPERCLIP_HOME = AgentsHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("Agents-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-recover-");
    cleanupDirs.add(AgentsHome);
    process.env.PAPERCLIP_HOME = AgentsHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("Agents-agent-instructions-stale-root-");
    cleanupDirs.add(AgentsHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("Agents-agent-instructions-heal-write-stale-");
    cleanupDirs.add(AgentsHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = AgentsHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("Agents-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(AgentsHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = AgentsHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const AgentsHome = await makeTempDir("Agents-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("Agents-agent-instructions-partial-root-");
    cleanupDirs.add(AgentsHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = AgentsHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
