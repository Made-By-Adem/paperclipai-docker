...
const CONFIG_PATH = process.env.PAPERCLIP_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".Agents/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "Agents-owner-password";
const INVITED_PASSWORD = "Agents-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@Agents.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@Agents.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("Agents_E2E_DATA_DIR or Agents_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Agents" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Agents account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Agents instance" })).toBeVisible();
