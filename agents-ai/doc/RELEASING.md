...
Maintainer runbook for shipping AgentsAI across npm, GitHub, and the website-facing changelog surface.
...
AgentsAI uses calendar versions that still fit semver syntax:
...
2. **npm** — `agentsai` and public workspace packages are published
...
# or
npx agentsai@canary onboard --data-dir "$(mktemp -d /tmp/agentsai-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/agentsai/agentsai/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for AgentsAI. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
gh workflow run release-smoke.yml -f agentsai_version=canary
gh workflow run release-smoke.yml -f agentsai_version=latest
```

Minimum checks:

- `npx agentsai@canary onboard` installs
