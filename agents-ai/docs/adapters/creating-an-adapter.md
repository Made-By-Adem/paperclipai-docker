...
Build a custom adapter to connect AgentsAI to any agent runtime.
...
|---|---|---|
| Source | Inside `agentsai-fork` | Separate npm package |
| Distribution | Ships with AgentsAI | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying AgentsAI's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@agentsai/adapter-utils/server-utils`
2. Build environment with `buildAgentsAIEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@agentsai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildAgentsAIEnv(agent)` | `@agentsai/adapter-utils/server-utils` | Inject `PAPERCLIP_*` env vars |
| `renderTemplate(tpl, data)` | `@agentsai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@agentsai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@agentsai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in AgentsAI's source:
...
`format-event.ts` — pretty-prints stdout for `agentsai run --watch` using `picocolors`.
...
With these flags set, the AgentsAI UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no AgentsAI source changes required.
...
Make AgentsAI skills discoverable to your agent runtime without writing to the agent's working directory:
