...
AgentsAI already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model AgentsAI should implement next.

## Product Goals

AgentsAI should let operators:
...
For the next implementation pass, AgentsAI should enforce these defaults:
...
AgentsAI should separate subscription-included usage from billed spend:
...
AgentsAI should have two threshold classes:
...
AgentsAI must know that a project is budget-paused and enforce it.
...
AgentsAI must also block execution before new work starts.
