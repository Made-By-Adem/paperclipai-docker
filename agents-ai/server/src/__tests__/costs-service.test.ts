...
import { createDb, companies, agents, costEvents, financeEvents, projects } from "@agentsai/db";
...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Agents-costs-service-");
