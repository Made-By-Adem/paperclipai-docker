...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    ...
    path.join(process.env.HOME ?? "/tmp", ".Agents-worktrees", "instances", "live-ssh-test");
  return path.join(instanceRoot, "env-lab", "ssh-fixture", "state.json");
}

/** Attempt to build config from explicit Agents_ENV_LIVE_SSH_* env vars. */
 ...
 * 1. Explicit Agents_ENV_LIVE_SSH_* env vars
...
const describeLiveSsh = (() => {
  ...
  if (process.env.PAPERCLIP_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set Agents_ENV_LIVE_SSH_HOST, Agents_ENV_LIVE_SSH_USERNAME, and Agents_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove Agents_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      throw new Error(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set Agents_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
    }
