 ...
 *
 * Plugin bundles are built with `external: ["@agentsai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__AgentsPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__AgentsPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__AgentsPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__AgentsPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@agentsai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@agentsai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agentsai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@agentsai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agentsai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__AgentsPluginBridge__) {
 ...
 * self-register via `window.AgentsPlugins.registerReactComponent()`.
 ...
 * of `@agentsai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
