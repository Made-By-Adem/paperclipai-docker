...
import { execute } from "@agentsai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects Agents env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Agents-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the Agents heartbeat.");
      expect(promptArg).toContain("Agents runtime note:");
      ...
      expect(invocationPrompt).toContain("Agents API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Agents-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Agents-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Agents Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the Agents heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
