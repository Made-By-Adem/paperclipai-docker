...
usage() {
  cat <<'EOF'
Usage:
  scripts/agentsai-issue-update.sh [--issue-id ID] [--status STATUS] [--comment TEXT] [--dry-run]
...
Examples:
  scripts/agentsai-issue-update.sh --issue-id "$PAPERCLIP_TASK_ID" --status in_progress <<'MD'
  ...
  scripts/agentsai-issue-update.sh --issue-id "$PAPERCLIP_TASK_ID" --status done --dry-run <<'MD'
  Done
