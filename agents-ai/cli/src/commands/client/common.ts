...
import { ApiRequestError, AgentsApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Agents config file")
    .option("-d, --data-dir <path>", "Agents data directory root (isolates state from ~/.Agents)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Agents API")
...
export function resolveCommandContext(
  ...
  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set Agents_COMPANY_ID, or set context profile companyId via `Agentsai context set`.",
    );
  }

  const api = new AgentsApiClient({
    apiBase,
    apiKey,
