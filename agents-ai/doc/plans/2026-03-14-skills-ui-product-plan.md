...
This document defines the product and UI plan for skill management in AgentsAI.
...
- AgentsAI company import/export can include skills as package content
...
3. The UI must always tell the truth about what AgentsAI knows:
   - desired state in AgentsAI
...
AgentsAI should treat skills at two scopes:
...
- created locally inside AgentsAI later
...
7. “Understand what AgentsAI knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a AgentsAI-specific package layout
...
This matters because Codex and similar adapters may already have local skills that AgentsAI did not install.

These should be clearly marked:

- external
- not managed by AgentsAI
...
- `desired_only`: AgentsAI wants it, adapter does not show it yet
- `external`: adapter has it but AgentsAI does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, AgentsAI only tracks desired state
...
- AgentsAI can still track desired skills
...
- AgentsAI can see adapter skills
...
- `.agentsai.yaml` may add AgentsAI-specific fidelity, but should not replace the base shortname association model
...
- `.agentsai.yaml` may add AgentsAI-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
AgentsAI should have a company-scoped skill model or managed package model representing:
...
AgentsAI should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too AgentsAI-specific.
...
That gives AgentsAI one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
