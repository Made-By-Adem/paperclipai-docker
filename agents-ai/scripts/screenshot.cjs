#!/usr/bin/env node
/**
 * Screenshot utility for AgentsAI UI.
 *
 * Reads the board token from ~/.agentsai/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/agentsai-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".agentsai/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.agentsai/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the AgentsAI origin only
