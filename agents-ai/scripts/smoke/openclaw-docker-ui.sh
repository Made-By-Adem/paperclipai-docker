...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.agentsai-openclaw.override.yml"
...
agentsai_base_url="$(detect_agentsai_base_url || true)"
...
AgentsAI URL for OpenClaw container:
EOF
  if [[ -n "$agentsai_base_url" ]]; then
    cat <<EOF
  $agentsai_base_url
...
EOF
  ...
  If AgentsAI rejects the host, run on host machine:
    pnpm agentsai allowed-hostname host.docker.internal
  Then restart AgentsAI and re-run this script.
EOF
  fi
  cat <<EOF
