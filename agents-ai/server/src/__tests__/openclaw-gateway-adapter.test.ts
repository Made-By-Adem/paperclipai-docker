...
import { execute, testEnvironment } from "@agentsai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@agentsai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("Agents:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("Agents_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("Agents_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Agents Wake Payload");
