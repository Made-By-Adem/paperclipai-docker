...
const Agents_SESSIONS_DIR = path.join(os.homedir(), ".pi", "Agentss");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[Agents] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[Agents] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[Agents] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Agents-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(Agents_SESSIONS_DIR, { recursive: true });
  return Agents_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(Agents_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readAgentsRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveAgentsDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // invoke skill binaries (e.g. `Agents-get-issue`) by name. Without this,
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[Agents] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      ...
      executionTargetIsRemote
        ? `[Agents] Pi session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`
        : `[Agents] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[Agents] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderAgentsWakePrompt(context.AgentsWake, { resumedSession: canResumeSession });
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[Agents] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
      );
