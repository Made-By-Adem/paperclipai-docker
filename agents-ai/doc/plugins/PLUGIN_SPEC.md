# AgentsAI Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for AgentsAI's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main AgentsAI app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary AgentsAI HTTP APIs directly.
...
AgentsAI plugin design is based on the following assumptions:

1. AgentsAI is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core AgentsAI business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by AgentsAI core.
...
2. Let plugins add major capabilities without editing AgentsAI core.
...
4. Depend on project-local plugin folders such as `.agentsai/plugins`.
...
The single AgentsAI deployment an operator installs and controls.

### 5.2 Company

A first-class AgentsAI business object inside the instance.
...
A trusted in-process extension loaded directly by AgentsAI core.
...
An installable instance-wide extension package loaded through the AgentsAI plugin runtime.
...
AgentsAI has two extension classes.
...
AgentsAI already has a concrete workspace model:
...
Plugins live under the AgentsAI instance directory.

Suggested layout:

- `~/.agentsai/instances/default/plugins/package.json`
- `~/.agentsai/instances/default/plugins/node_modules/`
- `~/.agentsai/instances/default/plugins/.cache/`
- `~/.agentsai/instances/default/data/plugins/<plugin-id>/`
...
AgentsAI should add CLI commands:

- `pnpm agentsai plugin list`
- `pnpm agentsai plugin install <package[@version]>`
- `pnpm agentsai plugin uninstall <plugin-id>`
- `pnpm agentsai plugin upgrade <plugin-id> [version]`
- `pnpm agentsai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@agentsai/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface AgentsAIPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that AgentsAI agents can use during runs.
...
- AgentsAI server starts one worker process per installed plugin
...
Receives one typed AgentsAI domain event.
...
export function definePlugin(definition: PluginDefinition): AgentsAIPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: AgentsAIPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@agentsai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@agentsai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@agentsai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@agentsai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@agentsai/plugin-sdk/ui`
...
The `@agentsai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the AgentsAI secret provider system rather than a plain text input
...
1. Core AgentsAI data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing AgentsAI objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual AgentsAI product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if AgentsAI later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing AgentsAI secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm agentsai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the AgentsAI server. This is a normative requirement, not optional.
...
### 27.1 `@agentsai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@agentsai/plugin-test-harness";
...
For developing a plugin against a running AgentsAI instance:

- The operator installs the plugin from a local path: `pnpm agentsai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-agentsai-plugin`) that scaffolds:

- `package.json` with correct `agentsaiPlugin` keys
...
- `@agentsai/plugin-workspace-files`
- `@agentsai/plugin-terminal`
- `@agentsai/plugin-git`
- `@agentsai/plugin-linear`
- `@agentsai/plugin-github-issues`
- `@agentsai/plugin-grafana`
- `@agentsai/plugin-runtime-processes`
- `@agentsai/plugin-stripe`
...
- `@agentsai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@agentsai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@agentsai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@agentsai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@agentsai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@agentsai/plugin-sdk/ui`
...
- `@agentsai/plugin-test-harness`
- `create-agentsai-plugin` starter template
...
AgentsAI should not implement a generic in-process hook bag modeled directly after local coding tools.

AgentsAI should implement:
...
That is the complete target design for the AgentsAI plugin system.
