...
import { getAdapterSessionManagement } from "@agentsai/adapter-utils";
import {
...
} from "@agentsai/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@agentsai/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@agentsai/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@agentsai/adapter-cursor-local";
import {
...
} from "@agentsai/adapter-gemini-local/server";
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@agentsai/adapter-gemini-local";
import {
  ...
  listOpenCodeModels,
} from "@agentsai/adapter-opencode-local/server";
import {
  agentConfigurationDoc as openCodeAgentConfigurationDoc,
  models as openCodeModels,
} from "@agentsai/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@agentsai/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@agentsai/adapter-openclaw-gateway";
...
import {
  ...
  listPiModels,
} from "@agentsai/adapter-pi-local/server";
import {
  agentConfigurationDoc as piAgentConfigurationDoc,
} from "@agentsai/adapter-pi-local";
import {
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.Agents_API_KEY === "string" && existingEnv.Agents_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Agents API safety rule:",
      "Use Authorization: Bearer $Agents_API_KEY on every Agents API request.",
      "Use X-Agents-Run-Id: $Agents_RUN_ID on every Agents API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Agents API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { Agents_API_KEY: normalizedCtx.authToken } : {}),
        Agents_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[Agents] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[Agents] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[Agents] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[Agents] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
