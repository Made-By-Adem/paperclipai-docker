...
describe("Agents skill utils", () => {
  ...
  it("lists bundled runtime skills from ./skills without pulling in .agents/skills", async () => {
    const root = await makeTempDir("Agents-skill-roots-");
    cleanupDirs.add(root);

    const moduleDir = path.join(root, "a", "b", "c", "d", "e");
    await fs.mkdir(moduleDir, { recursive: true });
    await fs.mkdir(path.join(root, "skills", "Agents"), { recursive: true });
    await fs.mkdir(path.join(root, "skills", "Agents-create-agent"), { recursive: true });
    await fs.mkdir(path.join(root, ".agents", "skills", "release"), { recursive: true });

    const entries = await listAgentsSkillEntries(moduleDir);

    expect(entries.map((entry) => entry.key)).toEqual([
      "Agentsai/Agents/Agents",
      "Agentsai/Agents/Agents-create-agent",
    ]);
    expect(entries.map((entry) => entry.runtimeName)).toEqual([
      "Agents",
      "Agents-create-agent",
    ]);
    expect(entries[0]?.source).toBe(path.join(root, "skills", "Agents"));
    expect(entries[1]?.source).toBe(path.join(root, "skills", "Agents-create-agent"));
  });

  it("marks skills with required: false in SKILL.md frontmatter as optional", async () => {
    const root = await makeTempDir("Agents-skill-optional-");
    cleanupDirs.add(root);
    ...
    expect(entries[0]?.runtimeName).toBe("Agents");
    expect(entries[0]?.required).toBe(true);
    expect(entries[1]?.runtimeName).toBe("Agents-dev");
    expect(entries[1]?.required).toBe(false);
    expect(entries[1]?.requiredReason).toBeNull();
  });

  it("removes stale maintainer-only symlinks from a shared skills home", async () => {
    const root = await makeTempDir("Agents-skill-cleanup-");
    cleanupDirs.add(root);

    const skillsHome = path.join(root, "skills-home");
    const runtimeSkill = path.join(root, "skills", "Agents");
    ...
    await fs.symlink(runtimeSkill, path.join(skillsHome, "Agents"));
    await fs.symlink(customSkill, path.join(skillsHome, "release-notes"));
    await fs.symlink(staleMaintainerSkill, path.join(skillsHome, "release"));

    const removed = await removeMaintainerOnlySkillSymlinks(skillsHome, ["Agents"]);
