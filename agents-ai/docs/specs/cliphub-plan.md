# ClipHub: Marketplace for AgentsAI Team Configurations
...
> The "app store" for whole-company AI teams — pre-built AgentsAI configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for AgentsAI-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | AgentsAI company (agents, projects, governance) |
...
A complete AgentsAI company configuration:
...
Single-agent configurations designed to plug into a AgentsAI org:
...
Portable skill files that any AgentsAI agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AgentsAI's skill loading system
...
interface Listing {
  ...
  agentsaiVersionMin: string;     // Minimum AgentsAI version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AgentsAI company
...
| `POST` | `/api/listings/:id/install` | Install to AgentsAI company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AgentsAI company (or create new)
  → ClipHub API calls AgentsAI API to:
  ...
  → Redirect to AgentsAI dashboard with new team running
```
...
### 5.3 Creator: Export from AgentsAI → Publish

```
Running AgentsAI company → "Export as Blueprint" (CLI or UI)
  → AgentsAI exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AgentsAI brand blue/purple)
...
| Install flow | One-click deploy to AgentsAI company |
...
{
  "targetCompanyId": "uuid",         // Existing AgentsAI company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `agentsai-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as AgentsAI
- **Backend**: Node.js API (or extend AgentsAI server)
- **Database**: Postgres (can share AgentsAI's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with AgentsAI auth (or OAuth2)

### 9.2 Integration with AgentsAI

ClipHub can be:
- **Option A**: A separate app that calls AgentsAI's API to install blueprints
- **Option B**: A built-in section of the AgentsAI UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AgentsAI UI and API.
...
- [ ] Install flow: blueprint → AgentsAI company
...
- [ ] "Export from AgentsAI" CLI command
