...
type OnboardDefaults = Pick<AgentsConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or Agents_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "Agents_PUBLIC_URL",
  "DATABASE_URL",
  "Agents_DB_BACKUP_ENABLED",
  "Agents_DB_BACKUP_INTERVAL_MINUTES",
  "Agents_DB_BACKUP_RETENTION_DAYS",
  "Agents_DB_BACKUP_DIR",
  "Agents_DEPLOYMENT_MODE",
  "Agents_DEPLOYMENT_EXPOSURE",
  "Agents_BIND",
  "Agents_BIND_HOST",
  "Agents_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "Agents_ALLOWED_HOSTNAMES",
  "Agents_AUTH_BASE_URL_MODE",
  "Agents_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "Agents_STORAGE_PROVIDER",
  "Agents_STORAGE_LOCAL_DIR",
  "Agents_STORAGE_S3_BUCKET",
  "Agents_STORAGE_S3_REGION",
  "Agents_STORAGE_S3_ENDPOINT",
  "Agents_STORAGE_S3_PREFIX",
  "Agents_STORAGE_S3_FORCE_PATH_STYLE",
  "Agents_SECRETS_PROVIDER",
  "Agents_SECRETS_STRICT_MODE",
  "Agents_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<AgentsConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveAgentsInstanceId();
  ...
  if (deploymentMode === "local_trusted" && process.env.PAPERCLIP_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "Agents_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<AgentsConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" Agentsai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAgentsInstanceId());
  p.log.message(
    pc.dim(
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Agents install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("Agentsai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("Agents_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.PAPERCLIP_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("Agents_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("Agents_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: Agents_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("Agentsai run")}`,
        `Reconfigure later: ${pc.cyan("Agentsai configure")}`,
        `Diagnose setup: ${pc.cyan("Agentsai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    p.outro("Existing Agents setup is ready.");
    return;
  }
  ...
  let llm: AgentsConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `Agentsai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("Agentsai run")}`,
        `Then: ${pc.cyan("Agentsai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
