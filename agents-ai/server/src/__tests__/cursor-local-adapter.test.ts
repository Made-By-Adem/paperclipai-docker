...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@agentsai/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@agentsai/adapter-cursor-local/ui";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$Agents_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $Agents_API_KEY\"";
