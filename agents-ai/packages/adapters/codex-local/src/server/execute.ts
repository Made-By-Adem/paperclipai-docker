...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@agentsai/adapter-utils";
import {
...
async function isLikelyAgentsRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyAgentsRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyAgentsRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableAgentsSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyAgentsRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[Agents] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readAgentsRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyAgentsRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog(
            "stdout",
            `[Agents] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }

      const result = await ensureAgentsSkillSymlink(entry.source, target, linkSkill);
      if (result === "skipped") continue;

      await onLog(
        "stdout",
        `[Agents] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[Agents] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableAgentsSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const workspaceContext = parseObject(context.AgentsWorkspace);
  ...
  const workspaceHints = Array.isArray(context.AgentsWorkspaces)
    ? context.AgentsWorkspaces.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServiceIntents = Array.isArray(context.AgentsRuntimeServiceIntents)
    ? context.AgentsRuntimeServiceIntents.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServices = Array.isArray(context.AgentsRuntimeServices)
    ? context.AgentsRuntimeServices.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimePrimaryUrl = asString(context.AgentsRuntimePrimaryUrl, "");
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readAgentsRuntimeSkillEntries(config, __moduleDir);
  ...
  const preparedExecutionTargetRuntime = executionTargetIsRemote
    ? await (async () => {
        await onLog(
          "stdout",
          `[Agents] Syncing workspace and CODEX_HOME to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
  ...
  const remoteCodexHome = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.home ??
      path.posix.join(effectiveExecutionCwd, ".Agents-runtime", "codex", "home")
    : null;
  const hasExplicitApiKey =
    typeof envConfig.Agents_API_KEY === "string" && envConfig.Agents_API_KEY.trim().length > 0;
  const env: Record<string, string> = { ...buildAgentsEnv(agent) };
  env.Agents_RUN_ID = runId;
  const wakeTaskId =
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  const wakePayloadJson = stringifyAgentsWakePayload(context.AgentsWake);
  if (wakeTaskId) {
    env.Agents_TASK_ID = wakeTaskId;
  }
  if (wakeReason) {
    env.Agents_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.Agents_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.Agents_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.Agents_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.Agents_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (wakePayloadJson) {
    env.Agents_WAKE_PAYLOAD_JSON = wakePayloadJson;
  }
  applyAgentsWorkspaceEnv(env, {
  ...
  if (workspaceHints.length > 0) {
    env.Agents_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  }
  if (runtimeServiceIntents.length > 0) {
    env.Agents_RUNTIME_SERVICE_INTENTS_JSON = JSON.stringify(runtimeServiceIntents);
  }
  if (runtimeServices.length > 0) {
    env.Agents_RUNTIME_SERVICES_JSON = JSON.stringify(runtimeServices);
  }
  if (runtimePrimaryUrl) {
    env.Agents_RUNTIME_PRIMARY_URL = runtimePrimaryUrl;
  }
  const targetAgentsApiUrl = adapterExecutionTargetAgentsApiUrl(executionTarget);
  if (targetAgentsApiUrl) {
    env.Agents_API_URL = targetAgentsApiUrl;
  }
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[Agents] Codex session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[Agents] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[Agents] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
    }
  }
  const repoAgentsNote =
    "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; Agents does not currently suppress that discovery.";
  ...
  const renderedBootstrapPrompt =
    !sessionId && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderAgentsWakePrompt(context.AgentsWake, { resumedSession: Boolean(sessionId) });
  const shouldUseResumeDeltaPrompt = Boolean(sessionId) && wakePrompt.length > 0;
  const promptInstructionsPrefix = shouldUseResumeDeltaPrompt ? "" : instructionsPrefix;
  instructionsChars = promptInstructionsPrefix.length;
  const continuationSummary = parseObject(context.AgentsContinuationSummary);
  ...
  const sessionHandoffNote = asString(context.AgentsSessionHandoffMarkdown, "").trim();
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[Agents] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
    ...
    if (restoreRemoteWorkspace) {
      await onLog(
        "stdout",
        `[Agents] Restoring workspace changes from ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
