...
AgentsAI needs to support two very different but equally valid ways of working:
...
Today, AgentsAI already has the beginnings of this model:
...
- making AgentsAI feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing AgentsAI users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted AgentsAI deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning AgentsAI into a full code review product
...
This object must be recorded explicitly so that AgentsAI can:
...
AgentsAI should treat PRs as a type of work product linked back to:
...
AgentsAI already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted AgentsAI must be a first-class deployment mode

AgentsAI cannot assume that it is running on the same machine as the code.

In cloud deployments, AgentsAI may:
...
- `Runtime service`: a process or service AgentsAI owns or tracks for a workspace
...
It also lets cloud-hosted AgentsAI deployments point at codebases and remotes without pretending the AgentsAI host has direct filesystem access.
...
Without an explicit `execution workspace` record, AgentsAI has nowhere stable to attach:
...
AgentsAI needs a single place to show:
...
- AgentsAI-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without AgentsAI owning the execution host
...
## 1. AgentsAI host and execution host must be decoupled
...
without the AgentsAI host itself running local git or local preview processes.
...
AgentsAI should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for AgentsAI.
...
AgentsAI should keep workspace automation centered on a small set of hooks:
...
AgentsAI should provide the equivalent visibility in the project `Code` surface:
...
AgentsAI should treat execution workspaces as the natural home for:
...
AgentsAI should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
AgentsAI should continue treating cleanup policy as part of the core workspace model:
...
AgentsAI should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the AgentsAI host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

AgentsAI should borrow Worktrunk's good execution patterns while keeping the broader AgentsAI model:
...
That keeps AgentsAI focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted AgentsAI deployments remain first-class

That is a better fit for AgentsAI than either extreme:
