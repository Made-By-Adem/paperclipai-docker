...
describe("pi remote execution", () => {
  ...
  it("prepares the workspace, syncs Pi skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Agents-pi-remote-"));
    ...
    expect(String(result.sessionId)).toContain("/remote/workspace/.Agents-runtime/pi/sessions/");
    expect(prepareWorkspaceForSshExecution).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: "/remote/workspace/.Agents-runtime/pi/skills",
      followSymlinks: true,
    }));
    expect(runSshCommand).toHaveBeenCalledWith(
      expect.anything(),
      expect.stringContaining(".Agents-runtime/pi/sessions"),
      expect.anything(),
    );
    ...
    expect(call?.[2]).toContain("/remote/workspace/.Agents-runtime/pi/skills");
  ...
  it("resumes saved Pi sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Agents-pi-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      runtime: {
        sessionId: "/remote/workspace/.Agents-runtime/pi/sessions/session-123.jsonl",
        sessionParams: {
          sessionId: "/remote/workspace/.Agents-runtime/pi/sessions/session-123.jsonl",
          cwd: "/remote/workspace",
          remoteExecution: {
    ...
    expect(call?.[2]).toContain("--session");
    expect(call?.[2]).toContain("/remote/workspace/.Agents-runtime/pi/sessions/session-123.jsonl");
  });
});
