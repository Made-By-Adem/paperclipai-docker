...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const desiredSkills = resolveAgentsDesiredSkillNames(config, availableEntries);
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Agents management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readAgentsRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureAgentsSkillSymlink(available.source, target);
  }
