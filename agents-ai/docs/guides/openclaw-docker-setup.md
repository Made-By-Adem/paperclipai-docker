...
How to get OpenClaw running in a Docker container for local development and testing the AgentsAI OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

AgentsAI includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-agentsai-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a AgentsAI host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-agentsai-smoke`)
...
If your AgentsAI deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host AgentsAI server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed AgentsAI URL (typically `http://host.docker.internal:3100`).
- If AgentsAI rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm agentsai allowed-hostname host.docker.internal
```

Then restart AgentsAI and rerun the smoke script.
