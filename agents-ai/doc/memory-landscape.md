...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for AgentsAI.

## What AgentsAI Needs From This Survey

AgentsAI is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to AgentsAI runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest AgentsAI contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because AgentsAI is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for AgentsAI | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; AgentsAI should not assume every backend behaves like mem0 |
| [AWS Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html) | AWS-managed memory service | explicit short-term and long-term memories, actor/session/event APIs, memory strategies, namespace templates, optional self-managed extraction pipeline | strong example of provider-managed memory with clear scoped ids, retention controls, and standalone API access outside a single agent framework | AWS-hosted and IAM-centric; AgentsAI would still need its own company/run/comment provenance, cost rollups, and likely a plugin wrapper instead of baking AWS semantics into core |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract AgentsAI should standardize first |
...
| [memU](https://github.com/NevaMind-AI/memU) | proactive agent memory framework | file-system metaphor, proactive loop, intent prediction, always-on companion model | good source for when memory should trigger agent behavior, not just retrieval | proactive assistant framing is broader than AgentsAI's task-centric control plane |
| [Memori](https://github.com/MemoriLabs/Memori) | hosted memory fabric + SDK wrappers | registers against LLM SDKs, attribution via `entity_id` + `process_id`, sessions, cloud + BYODB | strong example of automatic capture around model clients | wrapper-centric design does not map 1:1 to AgentsAI's run / issue / comment lifecycle |
...
If AgentsAI does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why AgentsAI needs a layered contract instead of a single hard-coded engine.
...
AgentsAI should support both:

- provider-managed extraction
- AgentsAI-managed extraction with provider-managed storage / retrieval
...
AgentsAI should not require a single storage shape. It should require normalized references back to AgentsAI entities.
...
AgentsAI should make plain search the minimum contract and richer outputs optional capabilities.
...
AgentsAI needs both direct request/response operations and background maintenance hooks.

## AgentsAI-Specific Takeaways

### AgentsAI should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping AgentsAI entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the AgentsAI UI
...
AgentsAI does not need to standardize every feature from every provider. It needs:
...
AgentsAI should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   AgentsAI decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns AgentsAI memory requests into provider-specific calls.
...
without forcing AgentsAI itself to become a monolithic memory engine.
