...
const DEFAULT_Agents_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_Agents_EMBEDDED_POSTGRES_PORT,
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Agents ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const { dataDir, instance } = await createEmbeddedPostgresTestInstance(
    "Agents-embedded-postgres-probe-",
  );
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const adminConnectionString = `postgres://Agents:Agents@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "Agents");
    const connectionString = `postgres://Agents:Agents@127.0.0.1:${port}/Agents`;
