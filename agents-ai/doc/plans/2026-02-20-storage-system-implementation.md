...
Add a single storage subsystem for AgentsAI that supports:
...
- API serves attachment bytes through authenticated AgentsAI endpoints.
...
- Default local path is under instance root: `~/.agentsai/instances/<instanceId>/data/storage`.
...
- `agentsai onboard` writes a valid `storage` config block by default.
- `agentsai configure --section storage` can switch between local and s3 modes.
...
- `agentsai doctor` reports actionable storage status.
