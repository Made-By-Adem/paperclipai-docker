...
This document explains how AgentsAI interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

AgentsAI separates four concepts that are easy to blur together:
...
This is a hard invariant. AgentsAI is single-assignee by design.

## 3. Status Semantics

AgentsAI issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, AgentsAI may still need a wake path to ensure the assignee actually sees it
...
- AgentsAI can wake the assignee
- AgentsAI can track runs linked to the issue
- AgentsAI can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- AgentsAI can track the ownership and status
- AgentsAI cannot rely on heartbeat/run semantics to keep them moving
...
AgentsAI already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

AgentsAI uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. AgentsAI should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.

If a parent is truly waiting on a child, model that with blockers. Do not rely on the parent/child relationship alone.

## 7. Non-Terminal Issue Liveness Contract

For agent-owned, non-terminal issues, AgentsAI should never leave work in a state where nobody is responsible for the next move and nothing will wake or surface it.

This is a visibility contract, not an auto-completion contract. If AgentsAI cannot safely infer the next action, it should surface the ambiguity with a blocked state, a visible comment, or an explicit recovery issue. It must not silently mark work done from prose comments or guess that a dependency is complete.
...
An `in_review` issue is stalled when it has no typed participant, no pending interaction or approval, no user owner, no active run, no queued wake, and no explicit recovery issue. AgentsAI should surface that state as recovery work rather than silently completing the issue or leaving blocker chains parked indefinitely.
...
AgentsAI now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, AgentsAI queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, AgentsAI moves the issue to `blocked` and posts a visible comment
...
- AgentsAI queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, AgentsAI moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, AgentsAI now does four things in sequence:
...
An active run can still be unhealthy even when its process is `running`. AgentsAI treats prolonged output silence as a watchdog signal, not as proof that the run is failed.
...
AgentsAI uses three different recovery outcomes, depending on how much it can safely infer.
...
AgentsAI creates an explicit recovery issue when the system can identify a problem but cannot safely complete the work itself.
...
In these cases AgentsAI should leave a visible issue/comment trail instead of silently retrying.

## 12. What This Does Not Mean

These semantics do not change V1 into an auto-reassignment system.

AgentsAI still does not:
...
That is the execution contract AgentsAI should present to operators.
