...
const DEFERRED_WAKE_CONTEXT_KEY = "_AgentsWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const Agents_WAKE_PAYLOAD_KEY = "AgentsWake";
const Agents_HARNESS_CHECKOUT_KEY = "AgentsHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const REPO_ONLY_CWD_SENTINEL = "/__Agents_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readAgentsSkillSyncPreference(config);
  return writeAgentsSkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[Agents truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[Agents_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[Agents_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildAgentsWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    livenessContinuation: readNonEmptyString(input.contextSnapshot.livenessContinuationState) ||
      ...
      : null,
    checkedOutByHarness: input.contextSnapshot[Agents_HARNESS_CHECKOUT_KEY] === true,
...
export function buildAgentsTaskMarkdown(input: {
  issue: {
    id: string;
    identifier: string | null;
    title: string;
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; Agents will wake the assignee when blockers resolve";
  ...
  async function executeRun(runId: string) {
    ...
    if (
      ...
      try {
        ...
        context[Agents_HARNESS_CHECKOUT_KEY] = true;
      } catch (error) {
        if (!isCheckoutConflictError(error)) throw error;
        context[Agents_HARNESS_CHECKOUT_KEY] = false;
      }
    ...
    if (continuationSummary) {
      context.AgentsContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.AgentsContinuationSummary;
    }
    const AgentsWakePayload = await buildAgentsWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (AgentsWakePayload) {
      context[Agents_WAKE_PAYLOAD_KEY] = AgentsWakePayload;
    } else {
      delete context[Agents_WAKE_PAYLOAD_KEY];
    }
    const taskMarkdown = buildAgentsTaskMarkdown({
      issue: issueRef
        ? {
            id: issueRef.id,
            identifier: issueRef.identifier,
    ...
    if (issueRef) {
      ...
      delete context.AgentsIssue;
    }
    if (wakeCommentContext) {
      context.AgentsWakeComment = wakeCommentContext;
    } else {
      delete context.AgentsWakeComment;
    }
    if (taskMarkdown) {
      context.AgentsTaskMarkdown = taskMarkdown;
    } else {
      delete context.AgentsTaskMarkdown;
    }
    ...
    let runtimeConfig = {
      ...
      AgentsRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.AgentsWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.AgentsRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.AgentsRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.AgentsSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.AgentsSessionRotationReason = sessionCompaction.reason;
      context.AgentsPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.AgentsSessionHandoffMarkdown;
      delete context.AgentsSessionRotationReason;
      delete context.AgentsPreviousSessionId;
    }
    ...
    try {
      ...
      if (runScopedMentionedSkillKeys.length > 0) {
        await onLog(
          "stdout",
          `[Agents] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
        );
      }
      ...
      if (runtimeServices.length > 0) {
        context.AgentsRuntimeServices = runtimeServices;
        context.AgentsRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[Agents] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected Agents_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.AgentsRuntimeServices = combinedRuntimeServices;
        context.AgentsRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[Agents] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[Agents] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Agents automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "Agents automatically retried continuation for this assigned `in_progress` issue during terminal run " +
