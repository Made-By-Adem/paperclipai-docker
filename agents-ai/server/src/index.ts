...
export async function startServer(): Promise<StartedServer> {
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set Agents_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@Agents.local";
  ...
  if (config.databaseUrl) {
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://Agents:Agents@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "Agents");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://Agents:Agents@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "Agents");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: Agents");
    }
  
    const embeddedConnectionString = `postgres://Agents:Agents@127.0.0.1:${port}/Agents`;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Agents server failed to start");
    process.exit(1);
  });
}
