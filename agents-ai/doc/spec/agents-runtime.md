...
For `claude_local` and `codex_local`, AgentsAI assumes the CLI is already installed and authenticated on the host machine.
...
AgentsAI stores resumable session state per `(agent, taskKey, adapterType)`.
