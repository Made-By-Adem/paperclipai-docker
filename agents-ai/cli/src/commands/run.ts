...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveAgentsInstanceId(opts.instance);
  process.env.PAPERCLIP_INSTANCE_ID = instanceId;

  const homeDir = resolveAgentsHomeDir();
  ...
  loadAgentsEnvFile(configPath);

  p.intro(pc.bgCyan(pc.black(" Agentsai run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("Agentsai onboard")} once, then retry ${pc.cyan("Agentsai run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Agents server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@agentsai/server/src/index.ts")) {
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Agents dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @agentsai/server package
  try {
    const mod = await import("@agentsai/server");
    return await startServerFromModule(mod, "@agentsai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@agentsai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Agents server entrypoint.\n` +
          `Tried: ${devEntry}, @agentsai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Agents server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: AgentsConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Agents server entrypoint did not export startServer(): ${label}`);
  }
