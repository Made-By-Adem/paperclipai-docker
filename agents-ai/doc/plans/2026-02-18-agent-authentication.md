...
Agents need API keys to authenticate with AgentsAI. The current approach
...
**Trust model:** The adapter process runs on the same machine as the AgentsAI
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** AgentsAI generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a AgentsAI CLI
...
```
agentsai auth login
# Opens browser -> user confirms -> token stored at ~/.agentsai/credentials
```
...
managed by the AgentsAI server itself.
...
1. A company admin (user or agent) generates an **invite URL** from AgentsAI.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The AgentsAI SKILL.md (or a link to it)
   - What information AgentsAI needs from the agent (e.g. webhook URL, adapter
...
5. AgentsAI stores the pending registration.
...
7. On approval, AgentsAI provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** AgentsAI issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.agentsai.ing/skills/agentsai/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How AgentsAI should send you heartbeats",
...
1. Generate a AgentsAI invite link for the company.
...
4. A AgentsAI company member approves the new agent.
5. AgentsAI begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `agentsai auth login` for developer-managed remote agents.                                      |
