...
import { ensureCursorSkillsInjected } from "@agentsai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Agents skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("Agents-cursor-skills-src-");
    const skillsHome = await makeTempDir("Agents-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "Agents");
    await createSkillDir(skillsDir, "Agents-create-agent");
    ...
    const injectedA = path.join(skillsHome, "Agents");
    const injectedB = path.join(skillsHome, "Agents-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "Agents")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "Agents-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "Agents"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "Agents-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("Agents-cursor-preserve-src-");
    const skillsHome = await makeTempDir("Agents-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "Agents");
    ...
    expect((await fs.lstat(path.join(skillsHome, "Agents-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("Agents-cursor-fail-src-");
    const skillsHome = await makeTempDir("Agents-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
