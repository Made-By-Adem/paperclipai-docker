...
import type { AdapterRuntimeServiceReport } from "@agentsai/adapter-utils";
import type { Db } from "@agentsai/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@agentsai/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".Agents", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.Agents_WORKSPACE_CWD = input.worktreePath;
  env.Agents_WORKSPACE_PATH = input.worktreePath;
  env.Agents_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.Agents_WORKSPACE_BRANCH = input.branchName;
  env.Agents_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.Agents_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.Agents_WORKSPACE_SOURCE = input.base.source;
  env.Agents_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.Agents_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.Agents_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.Agents_PROJECT_ID = input.base.projectId ?? "";
  env.Agents_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.Agents_AGENT_ID = input.agent.id ?? "";
  env.Agents_AGENT_NAME = input.agent.name;
  env.Agents_COMPANY_ID = input.agent.companyId;
  env.Agents_ISSUE_ID = input.issue?.id ?? "";
  env.Agents_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.Agents_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.Agents_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.Agents_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.Agents_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.Agents_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.Agents_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.Agents_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.Agents_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.Agents_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.Agents_PROJECT_ID = input.workspace.projectId ?? "";
  env.Agents_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.Agents_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".Agents", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Agents", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
