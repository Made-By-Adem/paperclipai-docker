...
Audience: Operators setting up and running agents in AgentsAI

## 1. What this system does

Agents in AgentsAI do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-agentsai-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-agentsai-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), AgentsAI assumes the CLI is already installed and authenticated on the host machine.
...
AgentsAI stores session IDs for resumable adapters.
...
AgentsAI pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. AgentsAI surfaces this as a warning in environment tests, not a hard error.
