...
const devServerStatusFilePath = path.join(repoRoot, ".agentsai", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-agentsai-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".agentsai/dev-server-status.json",
]);
...
if (tailscaleAuth) {
  ...
  env.HOST = "0.0.0.0";
  console.log("[agentsai] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[agentsai] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  ...
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[agentsai] Command failed with code ${status.code}: pnpm --filter @agentsai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[agentsai] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[agentsai] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[agentsai] building plugin sdk...");
  ...
  if (result.code !== 0) {
    console.error("[agentsai] plugin sdk build failed");
