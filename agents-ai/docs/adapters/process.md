...
- Running a Python script that calls the AgentsAI API
...
1. AgentsAI spawns the configured command as a child process
2. Standard AgentsAI environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the AgentsAI API and perform work.
