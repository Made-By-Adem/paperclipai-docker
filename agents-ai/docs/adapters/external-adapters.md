...
summary: Build, package, and distribute adapters as plugins without modifying AgentsAI source
---

AgentsAI supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to AgentsAI's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `agentsai-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with AgentsAI | Published to npm or linked via `file:` |
| Updates | Requires AgentsAI release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@agentsai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `agentsai.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@agentsai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with AgentsAI vars injected
  const env = buildAgentsAIEnv(agent);
...
#### Available Helpers from `@agentsai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildAgentsAIEnv(agent)` | Inject `PAPERCLIP_*` environment variables |
...
```sh
# Via the AgentsAI UI
# Settings → Adapters → Install from npm → "my-agentsai-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-agentsai-adapter"}'
```
...
Local adapters are symlinked into AgentsAI's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.agentsai/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-agentsai-adapter",
...
```ts
import type { AdapterSessionCodec } from "@agentsai/adapter-utils";
...
Other AgentsAI users can then install your adapter by package name from the UI or API.
