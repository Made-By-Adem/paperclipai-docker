 ...
 *    (`~/.Agents/plugins/`) and `node_modules` for packages matching
 *    the `Agents-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Agents plugins.
 * Packages matching this pattern are considered Agents plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "Agents-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: AgentsPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.Agents/plugins/ local directory
  | "npm"               // npm packages matching Agents-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: AgentsPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the Agents-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "Agents-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AgentsPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Agents plugins installed as npm packages in the current
   * Node.js environment matching the "Agents-plugin-*" naming convention.
   *
   ...
   * the "AgentsPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Agents plugin.
   * Throws if the package is a Agents plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AgentsPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AgentsPluginManifestV1;
    newManifest: AgentsPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Agents plugin naming convention.
 * Accepts both the "Agents-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @agentsai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "AgentsPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const AgentsPlugin = pkgJson["AgentsPlugin"];
  if (
    AgentsPlugin !== null &&
    typeof AgentsPlugin === "object" &&
    !Array.isArray(AgentsPlugin)
  ) {
    const manifestRelPath = (AgentsPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: AgentsPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAgentsVersion;
}
 ...
 *   packageName: "Agents-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AgentsPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AgentsPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Agents plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<AgentsPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Agents plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAgentsPlugin = "AgentsPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAgentsPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AgentsPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @agentsai/shared exports). Run those workers through
