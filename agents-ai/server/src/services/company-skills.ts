...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("Agentsai/Agents/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveAgentsInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "Agents_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Agents skills are read-only.",
      sourceLabel: "Agents bundled",
      sourceBadge: "Agents",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "Agents_bundled";
      out.push({
        key: skill.key,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "Agents_bundled"
        && incomingKind === "github"
        && incomingOwner === "Agentsai"
        && incomingRepo === "Agents"
      ) {
        out.push(existing);
        continue;
