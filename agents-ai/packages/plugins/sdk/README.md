...
Official TypeScript SDK for AgentsAI plugin authors.

- **Worker SDK:** `@agentsai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@agentsai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@agentsai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@agentsai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@agentsai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@agentsai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@agentsai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@agentsai/plugin-sdk/ui/hooks` | Hooks only |
| `@agentsai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@agentsai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@agentsai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@agentsai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@agentsai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@agentsai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main AgentsAI app. They can call ordinary AgentsAI HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the AgentsAI instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@agentsai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@agentsai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core AgentsAI information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@agentsai/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:agentsai.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@agentsai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@agentsai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@agentsai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@agentsai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@agentsai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@agentsai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@agentsai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@agentsai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@agentsai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@agentsai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@agentsai/plugin-sdk/bundlers";
...
```bash
agentsai-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@agentsai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__agentsai__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__agentsai__/events` streams `reload` SSE events on UI build changes
