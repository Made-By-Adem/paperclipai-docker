...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@agentsai/db";
import type { DeploymentMode } from "@agentsai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  AgentsUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).AgentsUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.AgentsUpgradeContext = context;
