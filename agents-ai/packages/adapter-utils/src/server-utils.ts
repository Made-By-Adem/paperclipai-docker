...
function normalizeAgentsWakeContinuationSummary(
  value: unknown,
): AgentsWakeContinuationSummary | null {
...
function normalizeAgentsWakeLivenessContinuation(
  value: unknown,
): AgentsWakeLivenessContinuation | null {
...
function normalizeAgentsWakeChildIssueSummary(
  value: unknown,
): AgentsWakeChildIssueSummary | null {
...
function normalizeAgentsWakeBlockerSummary(
  value: unknown,
): AgentsWakeBlockerSummary | null {
...
function normalizeAgentsWakeTreeHoldSummary(
  value: unknown,
): AgentsWakeTreeHoldSummary | null {
...
function normalizeAgentsWakeExecutionPrincipal(
  value: unknown,
): AgentsWakeExecutionPrincipal | null {
...
function normalizeAgentsWakeExecutionStage(
  value: unknown,
): AgentsWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeAgentsWakeExecutionPrincipal(
    stage.currentParticipant,
  );
  const returnAssignee = normalizeAgentsWakeExecutionPrincipal(
...
export function normalizeAgentsWakePayload(
  value: unknown,
): AgentsWakePayload | null {
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeAgentsWakeExecutionStage(
    payload.executionStage,
  );
  const continuationSummary = normalizeAgentsWakeContinuationSummary(
    payload.continuationSummary,
  );
  const livenessContinuation = normalizeAgentsWakeLivenessContinuation(
  ...
  const activeTreeHold = normalizeAgentsWakeTreeHoldSummary(
  ...
  if (
    ...
    !normalizeAgentsWakeIssue(payload.issue)
  ) {
    return null;
  }
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "Agents_RESOLVED_COMMAND"] =
      resolvedCommand;
  }
...
export function buildAgentsEnv(agent: {
  id: string;
  companyId: string;
}): Record<string, string> {
...
export function sanitizeInheritedAgentsEnv(
...
export async function resolveAgentsSkillsDir(
  ...
  const candidates = [
    ...Agents_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) =>
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available from the Agents skills directory.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Agents cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredAgentsRuntimeSkills(
  value: unknown,
): AgentsSkillEntry[] {
...
export async function readAgentsRuntimeSkillEntries(
  ...
  const configuredEntries = normalizeConfiguredAgentsRuntimeSkills(
    config.AgentsRuntimeSkills,
  );
...
export function readAgentsSkillSyncPreference(
  config: Record<string, unknown>,
): {
  explicit: boolean;
  desiredSkills: string[];
...
export function resolveAgentsDesiredSkillNames(
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) =>
      canonicalizeDesiredAgentsSkillReference(reference, availableEntries),
    )
    .filter(Boolean);
