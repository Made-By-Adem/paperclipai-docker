...
## Scope
This plan is now **gateway-only**. AgentsAI supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses AgentsAI skill.
...
```json
{
  "agentsaiApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a AgentsAI task.
...
```bash
pnpm --filter @agentsai/server test:run -- openclaw-gateway
pnpm --filter @agentsai/server typecheck
pnpm --filter @agentsai/ui typecheck
pnpm --filter agentsai typecheck
```
