...
const nonServerProjects = [
  "@agentsai/shared",
  "@agentsai/db",
  "@agentsai/adapter-utils",
  "@agentsai/adapter-codex-local",
  "@agentsai/adapter-opencode-local",
  "@agentsai/ui",
  "agentsai",
];
...
function runVitest(args, label) {
  console.log(`\n[test:run] ${label}`);
  invocationIndex += 1;
  const testRoot = mkdtempSync(path.join(os.tmpdir(), `agentsai-vitest-${process.pid}-${invocationIndex}-`));
  const env = {
    ...process.env,
...
runVitest(
  ["--project", "@agentsai/server", ...excludeRouteArgs],
