...
AgentsAI supports two runtime modes:
...
AgentsAI now treats **bind** as a separate concern from auth:
...
pnpm agentsai onboard --yes --bind lan
pnpm agentsai run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, AgentsAI emits a startup warning with a one-time high-entropy claim URL.
