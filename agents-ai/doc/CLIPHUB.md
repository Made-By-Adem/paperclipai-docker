...
ClipHub is the public registry where people share, discover, and download AgentsAI company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to AgentsAI what a package registry is to a programming language. AgentsAI already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in AgentsAI — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own AgentsAI instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the AgentsAI spec. It contains:
...
**Install (fresh start):**
```
agentsai install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local AgentsAI instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within AgentsAI, export your company as a template, then publish:

```
agentsai export --template my-company
agentsai publish cliphub my-company
```
...
ClipHub is a **separate service** from AgentsAI itself. AgentsAI is self-hosted; ClipHub is a hosted registry that AgentsAI instances talk to.
...
| **AgentsAI CLI** | `agentsai install`, `agentsai publish`, `agentsai cliphub sync` — built into AgentsAI |
| **AgentsAI UI** | "Browse ClipHub" panel in the AgentsAI web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with AgentsAI) |
| Backend | TypeScript + Hono (consistent with AgentsAI) |
| Database | PostgreSQL |
...
4. Run `agentsai install cliphub:acme/lean-saas-shop`
5. AgentsAI creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in AgentsAI until it works well
2. Export: `agentsai export --template my-agency`
3. Publish: `agentsai publish cliphub my-agency`
...
3. Install just that agent: `agentsai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to AgentsAI

ClipHub is **not required** to use AgentsAI. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to AgentsAI what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `agentsai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in AgentsAI web UI
- [ ] `agentsai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with AgentsAI's own philosophy)
