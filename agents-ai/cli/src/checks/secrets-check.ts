...
export function secretsCheck(config: AgentsConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: "Run `Agentsai configure --section secrets` and set provider to local_encrypted",
    };
  }
  ...
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "Agents_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set Agents_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via Agents_SECRETS_MASTER_KEY",
      },
      config,
    );
  }
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set Agents_SECRETS_MASTER_KEY",
    };
  }
