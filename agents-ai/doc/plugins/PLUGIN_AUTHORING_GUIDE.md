...
This guide describes the current, implemented way to create a AgentsAI plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main AgentsAI app.
...
```bash
pnpm --filter @agentsai/create-agentsai-plugin build
node packages/plugins/create-agentsai-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the AgentsAI repo:

```bash
pnpm --filter @agentsai/create-agentsai-plugin build
node packages/plugins/create-agentsai-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@agentsai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@agentsai/plugin-sdk` from the local AgentsAI checkout into a `.agentsai-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into AgentsAI from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@agentsai/plugin-sdk/ui`
