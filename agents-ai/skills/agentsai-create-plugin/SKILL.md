---
name: agentsai-create-plugin
description: >
  Create new AgentsAI plugins with the current alpha SDK/runtime. Use when
...
# Create a AgentsAI Plugin

Use this skill when the task is to create, scaffold, or document a AgentsAI plugin.
...
node packages/plugins/create-agentsai-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the AgentsAI repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.agentsai-sdk/`:

```bash
pnpm --filter @agentsai/create-agentsai-plugin build
node packages/plugins/create-agentsai-plugin/dist/index.js @acme/plugin-name \
...
- is installed into AgentsAI from an absolute local path during development
