...
Yes, there is a likely root cause in AgentsAI's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, AgentsAI can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: AgentsAI already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- AgentsAI may think it cancelled the run because the parent process is gone
...
This work is successful when AgentsAI can:
...
- require every skill author to add bespoke browser teardown logic before AgentsAI can clean up correctly
...
- make the leak measurable from AgentsAI's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by AgentsAI
...
Treat this as a AgentsAI executor ownership bug, not an `agent-browser` bug.
