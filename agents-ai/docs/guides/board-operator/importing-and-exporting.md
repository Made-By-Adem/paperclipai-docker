...
AgentsAI companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .agentsai.yaml     # Adapter config, env inputs, routines
```
...
- **.agentsai.yaml** holds AgentsAI-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
agentsai company export <company-id> --out ./my-export
```
...
agentsai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
agentsai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
agentsai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.agentsai.yaml`
...
```sh
# From a local folder
agentsai company import ./my-export

# From a GitHub URL
agentsai company import https://github.com/org/repo

# From a GitHub subfolder
agentsai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
agentsai company import org/repo
agentsai company import org/repo/companies/acme
```
...
If `--target` is not specified, AgentsAI infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
agentsai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
agentsai company import org/company-templates/engineering-team \
  --target new \
...
```sh
agentsai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
agentsai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
agentsai company import ./package \
  --target new \
  --yes \
  --json
```
...
AgentsAI supports several GitHub URL formats:
