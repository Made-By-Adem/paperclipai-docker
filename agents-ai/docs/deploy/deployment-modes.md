...
AgentsAI supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm agentsai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, AgentsAI emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated PAPERCLIP_BIND=lan pnpm agentsai run
```
