...
import type { AgentsConfig } from "@agentsai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Agents worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.Agents_IN_WORKTREE !== "true") return null;

  const configPath = resolveAgentsConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.Agents_WORKTREE_NAME) ??
    nonEmpty(env.Agents_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.Agents_INSTANCE_ID) ??
    nonEmpty(env.Agents_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.Agents_HOME) ??
      nonEmpty(env.Agents_HOME) ??
      nonEmpty(env.Agents_WORKTREES_DIR) ??
      "~/.Agents-worktrees",
  );
...
function writeConfigFile(configPath: string, config: AgentsConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".Agents", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as AgentsConfig;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentsConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    Agents_HOME: context.homeDir,
    Agents_INSTANCE_ID: context.instanceId,
    Agents_CONFIG: context.configPath,
    Agents_CONTEXT: context.contextPath,
    Agents_IN_WORKTREE: "true",
    Agents_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: AgentsConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentsConfig;
  } catch {
    return;
  }
