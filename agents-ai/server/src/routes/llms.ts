...
import { AGENT_ICON_NAMES } from "@agentsai/shared";
...
export function llmRoutes(db: Db) {
  ...
  router.get("/llms/agent-configuration.txt", async (req, res) => {
    ...
    const lines = [
      "# Agents Agent Configuration Index",
      "",
      "Installed adapters:",
      ...
      "- Use the Agents-create-agent skill for end-to-end hiring: adapter reflection, config comparison, instruction source selection, icon choice, desiredSkills, sourceIssueId/sourceIssueIds, and approval follow-up.",
