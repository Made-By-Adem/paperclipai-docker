...
AgentsAI should not replace its workspace runtime service model with VS Code tasks.
...
- AgentsAI owns long-running workspace services and their desired state
- AgentsAI shows operators exactly which named thing they are starting or stopping
- AgentsAI distinguishes long-running services from one-shot jobs
...
This is directionally correct for AgentsAI because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and AgentsAI does not yet leverage existing workspace metadata.

## Recommendation

Keep AgentsAI runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in AgentsAI.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to AgentsAI service or job semantics should become runnable in AgentsAI.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than AgentsAI runtime services.
...
That creates a bad fit if AgentsAI tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace AgentsAI's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

AgentsAI should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command AgentsAI can resolve
...
- selected variable substitution when AgentsAI can resolve it safely from workspace context
- mapping task metadata into AgentsAI readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types AgentsAI cannot execute directly
...
AgentsAI needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by AgentsAI metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by AgentsAI
...
For a workspace with `cwd`, AgentsAI should look for `.vscode/tasks.json`.
...
AgentsAI should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - AgentsAI metadata marks it as a service, or
...
AgentsAI-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what AgentsAI would import and why.
...
Goal:
run supported imported service commands through the current AgentsAI supervisor.
...
Success condition:
imported service commands behave like native AgentsAI services once adopted.
...
- removal of AgentsAI's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting AgentsAI to VS Code and making the model worse for non-VS-Code repos
...
AgentsAI should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing AgentsAI workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
