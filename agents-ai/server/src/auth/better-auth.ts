...
import { buildCognitoPlugin, loadCognitoSsoConfig } from "./cognito-sso.js";
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins: string[]): BetterAuthInstance {
  ...
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or Agents_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=Agents-dev-secret in your .env file.",
    );
  }
  const publicUrl = process.env.PAPERCLIP_PUBLIC_URL ?? baseUrl;
  const isHttpOnly = publicUrl ? publicUrl.startsWith("http://") : false;

  const cognitoConfig = loadCognitoSsoConfig();
  const plugins = cognitoConfig ? [buildCognitoPlugin(cognitoConfig)] : [];
