...
function buildLivenessEscalationDescription(finding: IssueLivenessFinding) {
  ...
  return [
    "Agents detected a harness-level issue graph liveness incident.",
    "",
    "## Source",
    "",
...
function buildLivenessOriginalIssueComment(finding: IssueLivenessFinding, escalation: typeof issues.$inferSelect) {
  return [
    "Agents detected a harness-level liveness incident in this issue's dependency graph.",
    "",
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `Agents found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
  ...
  function buildStaleRunEvaluationDescription(input: {
    ...
    const blockers = input.evidence.blockers.length > 0
      ...
      : "- none detected";
    return [
      `Agents detected ${input.level} output silence on an active heartbeat run.`,
      "",
      "## Run",
      "",
  ...
  async function ensureSourceIssueBlockedByStaleEvaluation(input: {
    ...
    await issuesSvc.addComment(input.sourceIssue.id, [
      "Agents detected critical output silence on this issue's active run.",
      "",
  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    return [
      "Agents exhausted automatic recovery for an assigned issue and created this explicit recovery task.",
      "",
      "## Source",
      "",
      `- Source issue: ${sourceIssue}`,
  ...
  function buildRecoveryIssueInPlaceEscalationComment(input: {
    ...
    return [
      "Agents stopped automatic stranded-work recovery for this recovery issue.",
      "",
  ...
  async function escalateStrandedAssignedIssue(input: {
    ...
    const recoveryLine = recoveryIssue
      ...
      : [
        "",
        "- Recovery issue: none created because Agents could not find an invokable manager, creator, or executive owner with budget available.",
    ...
    for (const issue of candidates) {
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "Agents automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
      ...
      if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
        ...
        const updated = await escalateStrandedAssignedIssue({
          ...
          latestRun,
          comment:
            "Agents automatically retried continuation for this assigned `in_progress` issue after its live " +
