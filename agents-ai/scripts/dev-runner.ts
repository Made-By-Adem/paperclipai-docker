...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[Agents] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`Agentsai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".Agents", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-Agents-dev-server-status-token";
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[Agents] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[Agents] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[Agents] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  Agents_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.Agents_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.Agents_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.Agents_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.Agents_DEV_SERVER_STATUS_TOKEN;
  env.Agents_MIGRATION_PROMPT ??= "never";
  env.Agents_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[Agents] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.Agents_BIND = effectiveBind;
  if (bindHost) {
    env.Agents_BIND_HOST = bindHost;
  } else {
    delete env.Agents_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.Agents_DEPLOYMENT_MODE;
    delete env.Agents_DEPLOYMENT_EXPOSURE;
    delete env.Agents_AUTH_BASE_URL_MODE;
    console.log("[Agents] dev mode: local_trusted (bind=loopback)");
  } else {
    env.Agents_DEPLOYMENT_MODE = "authenticated";
    env.Agents_DEPLOYMENT_EXPOSURE = "private";
    env.Agents_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[Agents] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.Agents_BIND;
  delete env.Agents_BIND_HOST;
  delete env.Agents_DEPLOYMENT_MODE;
  delete env.Agents_DEPLOYMENT_EXPOSURE;
  delete env.Agents_AUTH_BASE_URL_MODE;
  console.log("[Agents] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[Agents] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@agentsai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[Agents] Command failed with code ${status.code}: pnpm --filter @agentsai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[Agents] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.Agents_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[Agents] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[Agents] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@agentsai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[Agents] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@agentsai/server", serverScript, ...forwardedArgs],
