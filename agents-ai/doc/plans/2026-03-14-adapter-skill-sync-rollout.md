...
This document defines the rollout plan for adapter-wide skill support in AgentsAI.
...
- AgentsAI stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

AgentsAI currently has these adapters:
...
1. Desired skills live in AgentsAI for every adapter.
...
These adapters have a stable local skills directory that AgentsAI can read and manage.
...
These adapters do not have a meaningful AgentsAI-owned persistent install state.
...
- desired skills stored in AgentsAI
...
- runtime already injects AgentsAI skills into `~/.cursor/skills`
...
- runtime already injects AgentsAI skills into `~/.gemini/skills`
...
- runtime already injects AgentsAI skills into `~/.pi/agent/skills`
...
- AgentsAI can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects AgentsAI skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- AgentsAI may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed AgentsAI skills that are clearly marked as AgentsAI-managed
...
- AgentsAI stores desired skills only
...
- avoid destructive wording unless AgentsAI can prove a skill is AgentsAI-managed
...
That gets AgentsAI from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
