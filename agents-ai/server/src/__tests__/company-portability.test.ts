...
import type { CompanyPortabilityFileEntry } from "@agentsai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "Agents",
        name: "Agents",
        description: "Agents coordination skill",
        markdown: "---\nname: Agents\ndescription: Agents coordination skill\n---\n\n# Agents\n",
        sourceType: "github",
        sourceLocator: "https://github.com/Agentsai/Agents/tree/master/skills/Agents",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/Agentsai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "Agentsai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/Agentsai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "Agentsai",
  ...
  it("exports referenced skills as stubs by default with sanitized Agents extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Agents"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${AgentsKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/Agentsai/Agents/Agents/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".Agents.yaml"]);
    expect(extension).toContain('schema: "Agents/v1"');
    ...
    expect(extension).not.toContain("AgentsSkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".Agents.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });

  it("exports default sidebar order into the Agents extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".Agents.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"])).toContain("# Agents");
    expect(asTextFile(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/Agentsai/Agents/Agents/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/Agentsai/Agents/Agents/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .Agents.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".Agents.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-Agents",
        companyId: "company-1",
        key: "Agentsai/Agents/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/Agentsai/Agents/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "Agents_bundled",
          owner: "Agentsai",
          repo: "Agents",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/Agentsai/Agents/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/Agentsai/Agents/release-changelog/SKILL.md"])).toContain("Agentsai/Agents/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/Agents",
            repoUrl: "https://github.com/Agentsai/Agents.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .Agents-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/Agentsai/Agents.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/Agents");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Agents",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "Agents-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/Agentsai/Agents.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".Agents.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/Agentsai/Agents.git"');
    expect(extension).toContain('projectWorkspaceKey: "Agents"');
  ...
  it("reads env inputs back from .Agents.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .Agents.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Agents routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "Agents-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Agents" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Agents"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .Agents.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Agents");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".Agents.yaml"] = `${exported.files[".Agents.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
