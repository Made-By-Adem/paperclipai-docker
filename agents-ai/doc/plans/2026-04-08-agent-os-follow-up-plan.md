...
Turn the `agent-os` research into a low-risk AgentsAI execution plan that preserves AgentsAI's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

AgentsAI should not absorb `agent-os` as a product model or orchestration layer.

AgentsAI should evaluate `agent-os` in three narrow areas:
...
This work is successful when AgentsAI has:
...
- a proposed runtime capability model that fits current AgentsAI adapters
...
- replace AgentsAI heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to AgentsAI core

## Existing AgentsAI integration points
...
- prove that AgentsAI can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep AgentsAI's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a AgentsAI-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some AgentsAI workloads
...
- identify where AgentsAI should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in AgentsAI
...
- runtime experimentation could blur the boundary between AgentsAI as control plane and AgentsAI as execution platform
