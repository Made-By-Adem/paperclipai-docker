# AgentsAI Memory Service Plan

## Goal

Define a AgentsAI memory service and surface API that can sit above multiple memory backends, while preserving AgentsAI's control-plane requirements:

- company scoping
- auditability
- provenance back to AgentsAI work objects
- budget and cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md`, the AWS AgentCore comparison captured in [PAP-1274](/PAP/issues/PAP-1274), and the current AgentsAI architecture in:
...
AgentsAI should add a company-scoped memory control plane with company default plus agent override resolution, shared hook delivery, and full operation attribution, while leaving extraction and storage semantics to built-ins and plugins.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing AgentsAI plugin runtime.

That keeps the core small and matches the broader AgentsAI direction that specialized knowledge systems live at the edges.

### 4. AgentsAI owns routing, provenance, and policy

Providers should not decide how AgentsAI entities map to governance.

AgentsAI core should own:
...
### 5. AgentsAI exposes shared hooks, providers own extraction

AgentsAI should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured AgentsAI source objects plus normalized metadata. The provider then decides how to extract from those objects.

AgentsAI should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
AgentsAI should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a AgentsAI target to a binding.
...
The normalized AgentsAI scope passed into a provider request.
...
A normalized trigger emitted by AgentsAI when something memory-relevant happens.
...
A normalized capture, record-write, query, browse, get, correction, or delete action performed through AgentsAI.

AgentsAI should log every memory operation whether the provider is local, plugin-backed, or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy AgentsAI's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for AgentsAI to browse, attribute, and audit what happened.
...
That lines up with the AgentsAI plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes AgentsAI should take from AWS are:
...
AWS exposes list and retrieve surfaces directly. AgentsAI should not make browse optional at the portable layer.
...
AgentsAI should add cursor-based pagination to:
...
AgentsAI should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. AgentsAI should too.
...
### 7. Keep AgentsAI provenance primary

AgentsAI should continue to center:
...
## What AgentsAI Should Persist

AgentsAI should not mirror the full provider memory corpus into Postgres unless the provider is a AgentsAI-managed local provider.

AgentsAI core should persist:
...
AgentsAI should expose one shared hook system for memory.
...
1. `pre_run_hydrate`
   Before an agent run starts, AgentsAI may call `query(... intent = "agent_preamble")` using the active binding.

2. `post_run_capture`
   After a run finishes, AgentsAI may call `capture(...)` with structured run output, excerpts, and provenance.

3. `issue_comment_capture`
   When enabled on the binding, AgentsAI may call `capture(...)` for selected issue comments.

4. `issue_document_capture`
   When enabled on the binding, AgentsAI may call `capture(...)` for selected issue documents.
...
AgentsAI should give agents both automatic recall and explicit tools, with simple guidance:
...
AgentsAI needs a first-class UI for memory, otherwise providers become black boxes.
...
AgentsAI should treat memory accounting as two related but distinct concerns:
...
- if a memory operation runs inside a normal AgentsAI agent heartbeat and the model usage is already counted on that run, do not create a duplicate `cost_event`
- instead, store the memory operation with `attributionMode = "included_in_run"` and link it to the related `heartbeatRunId`
- if a memory provider makes a direct metered model call outside the agent run accounting path, the provider must report usage and AgentsAI should create a `cost_event`
...
AgentsAI should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget or silently fails extraction is not acceptable in AgentsAI.
...
- it matches AgentsAI's local-first posture
- it is inspectable
...
- How much source payload should AgentsAI snapshot inside `memory_operations` for debugging without duplicating large transcripts?
...
- AgentsAI owns bindings, resolution, hooks, provenance, policy, and attribution.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives AgentsAI a stable memory service without locking the product to one memory philosophy or one vendor, and it integrates the AWS lessons without importing AWS's model into core.
