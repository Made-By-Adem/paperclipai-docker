...
function registerModuleMocks() {
  vi.doMock("@agentsai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@agentsai/adapter-opencode-local/server")>("@agentsai/adapter-opencode-local/server");
    return {
      ...actual,
  ...
  vi.doMock("@agentsai/shared/telemetry", () => ({
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@agentsai/shared/telemetry");
    ...
    vi.doUnmock("@agentsai/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/Agents-rce",
          },
        },
      }));
