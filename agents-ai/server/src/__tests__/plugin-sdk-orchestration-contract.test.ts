...
import type { Issue, AgentsPluginManifestV1 } from "@agentsai/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: AgentsPluginManifestV1["capabilities"]): AgentsPluginManifestV1 {
  return {
    id: "Agents.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:Agents.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:Agents.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:Agents.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:Agents.test-orchestration");
    await expect(
