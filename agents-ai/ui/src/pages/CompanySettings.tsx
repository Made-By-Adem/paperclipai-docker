                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        {environment.driver === "ssh" ? (
                          ...
                          <div className="text-xs text-muted-foreground">Runs on this Agents host.</div>
                        )}
                      </div>
            ...
            <div className="space-y-3">
              ...
              {environmentForm.driver === "ssh" ? (
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <Field label="Remote workspace path" hint="Absolute path that Agents will verify during SSH connection tests.">
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                      type="text"
                      placeholder="/Users/Agents/workspace"
...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Agents, then retry.
Suggested steps:
- choose a hostname that resolves to the Agents host from your runtime
- run: pnpm Agentsai allowed-hostname <host>
- restart Agents
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in Agents, restart, and retry.
Suggested command:
- pnpm Agentsai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Agents-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Agents. Test it. `
    : "";

  return `You're invited to join a Agents organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Agents, Agents must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Agents can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, Agents will generate and persist one during join so pairing approvals remain stable.
- Only use \`disableDeviceAuth=true\` for special environments where pairing cannot be completed.

Do NOT use \`/v1/responses\` or \`/hooks/*\` in this join flow.

Before you do anything, please respond to your user that you understand the instructions and you're going to work on them. Then do the step above in another session called "Agents-onboarding" and then tell your user when you're done. Update your user in intermediate steps along the way so they know what's going on.

Then after you've connected to Agents (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.
