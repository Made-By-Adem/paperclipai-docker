 ...
 */
import { pluginManifestV1Schema } from "@agentsai/shared";
import type { AgentsPluginManifestV1 } from "@agentsai/shared";
import { PLUGIN_API_VERSION } from "@agentsai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AgentsPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AgentsPluginManifestV1,
        };
      }
