...
import type { DeploymentExposure, DeploymentMode } from "@agentsai/shared";
...
import { cxAssistSsoRoutes } from "./auth/cx-assist-sso.js";
...
import { createHostClientHandlers } from "@agentsai/plugin-sdk";
...
export async function createApp(
  ...
  // Cross-domain SSO from cx-assist must be registered BEFORE the better-auth
  // catch-all so /api/auth/sso/handoff is handled by us, not Better Auth.
  app.use("/api/auth/sso", cxAssistSsoRoutes(db));
  ...
  if (opts.uiMode === "static") {
    ...
    if (uiDist) {
      ...
      console.warn("[Agents] UI dist not found; running in API-only mode");
    }
  }
