...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-onboard-"));
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-onboard-fresh-"));
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as AgentsConfig;
