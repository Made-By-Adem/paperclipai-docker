...
const Agents_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp Agents_HOME.
  webServer: {
    command: `pnpm Agentsai onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Agents home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      Agents_HOME,
      Agents_INSTANCE_ID: "playwright-e2e",
      Agents_BIND: "loopback",
      Agents_DEPLOYMENT_MODE: "local_trusted",
      Agents_DEPLOYMENT_EXPOSURE: "private",
    },
  },
