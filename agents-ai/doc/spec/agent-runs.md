...
This spec defines how AgentsAI actually runs agents while staying runtime-agnostic.
...
1. AgentsAI is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, AgentsAI manages lifecycle, and on exit AgentsAI parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `agentsai.skill` (shared AgentsAI skill text block)
