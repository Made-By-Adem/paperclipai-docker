...
import { resolveAgentsEnvPath } from "./paths.js";
...
const Agents_ENV_FILE_PATH = resolveAgentsEnvPath();
if (existsSync(Agents_ENV_FILE_PATH)) {
  loadDotenv({ path: Agents_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(Agents_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(Agents_ENV_FILE_PATH)
  : CWD_ENV_PATH === Agents_ENV_FILE_PATH;
...
export function loadConfig(): Config {
  ...
  const storageS3Bucket = process.env.PAPERCLIP_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "Agents";
