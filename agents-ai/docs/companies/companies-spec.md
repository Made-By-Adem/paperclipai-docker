...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only AgentsAI.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.agentsai.yaml`
...
- AgentsAI must not require extra top-level fields for skill validity
- AgentsAI-specific extensions must live under `metadata.agentsai` or `metadata.sources`
...
For AgentsAI, the preferred fidelity extension is:
...
- issue/task AgentsAI-only metadata
...
- AgentsAI tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested AgentsAI shape:
...
Additional rules for AgentsAI exporters:
...
- omit empty and default-valued AgentsAI fields when possible
...
- AgentsAI exporters should emit `.agentsai.yaml` as a sidecar by default
...
AgentsAI can map this spec to its runtime model like this:
...
- AgentsAI extension:
  - `.agentsai.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other AgentsAI-specific fidelity

Inline AgentsAI-only metadata that must live inside a shared markdown file should use:

- `metadata.agentsai`

That keeps the base format broader than AgentsAI.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only AgentsAI.

## 20. Cutover

AgentsAI should cut over to this markdown-first package model as the primary portability format.

`agentsai.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For AgentsAI, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
