...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://Agents:Agents@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "Agents");
      const connectionString = `postgres://Agents:Agents@127.0.0.1:${embeddedHandle.port}/Agents`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set Agents_COMPANY_ID.");
  }
...
export function registerRoutineCommands(program: Command): void {
  ...
  routinesCommand
    ...
    .option("-d, --data-dir <path>", "Agents data directory root (isolates state from ~/.Agents)")
