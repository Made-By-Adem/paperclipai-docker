 ...
 * channel.
 *
 * The Agents plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@agentsai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Agents messages.
 */
...
/**
 * Agents plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Agents instance. */
    instanceId: string;
    /** Semver version of the running Agents host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
