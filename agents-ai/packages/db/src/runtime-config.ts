...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "Agents-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveAgentsHomeDir(): string {
  const envHome = process.env.PAPERCLIP_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".Agents");
}

function resolveAgentsInstanceId(): string {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveAgentsHomeDir(), "instances", resolveAgentsInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".Agents", CONFIG_BASENAME);
...
function resolveAgentsEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveAgentsEnvPath(configPath);
