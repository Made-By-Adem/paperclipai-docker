...
This document defines the next-stage plan for AgentsAI company import/export.

The core shift is:

- move from a AgentsAI-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside AgentsAI.
...
AgentsAI already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just AgentsAI
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `agentsai.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.agentsai.yaml` sidecar for high-fidelity AgentsAI-specific details
...
11. adapter-aware skill sync surfaces so AgentsAI can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point AgentsAI at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a AgentsAI sidecar.
...
AgentsAI must not redefine `SKILL.md`.
...
- AgentsAI-specific fidelity lives in `.agentsai.yaml`
- AgentsAI may resolve and install `SKILL.md` packages, but it must not require a AgentsAI-only skill format
...
### 5.4 Base Package Vs AgentsAI Extension
...
- base package:
  ...
  - no AgentsAI-only runtime fields by default
- AgentsAI extension:
  - `.agentsai.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by AgentsAI tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`agentsai.manifest.json` is not part of the future package direction.
...
AgentsAI import/export should support these entity kinds:
...
- AgentsAI can treat “this set of agents came from team package X” as the imported-team model
...
AgentsAI should support source references in package metadata with:
...
AgentsAI should define a new adapter surface area around skills:
...
Planned AgentsAI behavior:

- if an adapter supports read, AgentsAI should show current skills in the UI
- if an adapter supports write, AgentsAI should let the user enable/disable imported skills
- if an adapter supports sync, AgentsAI should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.agentsai.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.agentsai.yaml` alongside the base package
...
## 10. Storage Model Inside AgentsAI
...
AgentsAI should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `agentsai company export <company-id> --out <path>`
- `agentsai company import <path-or-url> --dry-run`
- `agentsai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in AgentsAI storage, or only referenced at import time?
...
3. Should AgentsAI support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps AgentsAI aligned with:
