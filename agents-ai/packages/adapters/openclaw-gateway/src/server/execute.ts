...
const DEFAULT_CLIENT_VERSION = "Agents";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "Agents";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`Agents:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`Agents:issue:${input.issueId}`, input.agentId);
  }
...
function resolveAgentsApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/Agents-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildAgentsEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const AgentsApiUrlOverride = resolveAgentsApiUrlOverride(ctx.config.AgentsApiUrl);
  const AgentsEnv: Record<string, string> = {
    ...buildAgentsEnv(ctx.agent),
    Agents_RUN_ID: ctx.runId,
  };

  if (AgentsApiUrlOverride) {
    AgentsEnv.Agents_API_URL = AgentsApiUrlOverride;
  }
  if (wakePayload.taskId) AgentsEnv.Agents_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) AgentsEnv.Agents_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) AgentsEnv.Agents_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) AgentsEnv.Agents_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) AgentsEnv.Agents_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    AgentsEnv.Agents_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  AgentsEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/Agents-claimed-api-key.json";
  const orderedKeys = [
    "Agents_RUN_ID",
    "Agents_AGENT_ID",
    "Agents_COMPANY_ID",
    ...
    "Agents_TASK_ID",
    "Agents_WAKE_REASON",
    "Agents_WAKE_COMMENT_ID",
    "Agents_APPROVAL_ID",
    "Agents_APPROVAL_STATUS",
    "Agents_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = AgentsEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = AgentsEnv.Agents_API_URL ?? "<set Agents_API_URL>";

  const lines = [
    "Agents wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `Agents_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load Agents_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $Agents_API_KEY on every API call.",
    "- Use X-Agents-Run-Id: $Agents_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: Agents_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$Agents_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$Agents_COMPANY_ID/issues?assigneeAgentId=$Agents_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardAgentsPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  AgentsEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateAgents = parseObject(payloadTemplate.Agents);
  const workspace = asRecord(ctx.context.AgentsWorkspace);
  const workspaces = Array.isArray(ctx.context.AgentsWorkspaces)
    ? ctx.context.AgentsWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.AgentsRuntimeServiceIntents)
    ? ctx.context.AgentsRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardAgents: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: AgentsEnv.Agents_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.AgentsWake);
  if (Object.keys(structuredWake).length > 0) {
    standardAgents.wake = structuredWake;
  }

  if (workspace) {
    standardAgents.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardAgents.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardAgents.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "Agents-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const AgentsEnv = buildAgentsEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderAgentsWakePrompt(ctx.context.AgentsWake);
  const structuredWakeJson = stringifyAgentsWakePayload(ctx.context.AgentsWake);
  ...
  const AgentsPayload = buildStandardAgentsPayload(ctx, wakePayload, AgentsEnv, payloadTemplate);
  ...
  agentParams.Agents = AgentsPayload;
