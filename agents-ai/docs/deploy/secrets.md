...
AgentsAI encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.
...
```
~/.agentsai/instances/default/secrets/master.key
```
