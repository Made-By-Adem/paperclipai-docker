...
AgentsAI's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends AgentsAI's API, UI, and data model | Yes |
...
modules/
  observability/
    agentsai.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@agentsai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@agentsai/core";
...
```
1. Scan modules/ directory for agentsai.module.json manifests
...
```jsonc
// agentsai.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@agentsai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@agentsai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:agentsai/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.agentsai.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm agentsai store list                    # browse available modules and templates
pnpm agentsai store install <module-id>     # install a module
pnpm agentsai store import <template-id>    # import a company template
pnpm agentsai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one AgentsAI instance | `server:started` |
...
4. **Module config** — `agentsai.config.json` with per-module config, env var interpolation
...
10. **`@agentsai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
