...
import { asString, parseObject } from "@agentsai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Agents server host.",
        });
      }
    } catch (err) {
      checks.push({
