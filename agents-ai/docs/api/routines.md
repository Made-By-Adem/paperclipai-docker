...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-AgentsAI-Signature` + `X-AgentsAI-Timestamp` header pair matching the trigger's signing mode.
