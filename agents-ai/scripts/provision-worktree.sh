...
agentsai_home="${PAPERCLIP_HOME:-$HOME/.agentsai}"
agentsai_instance_id="${PAPERCLIP_INSTANCE_ID:-default}"
agentsai_dir="$worktree_cwd/.agentsai"
worktree_config_path="$agentsai_dir/config.json"
worktree_env_path="$agentsai_dir/.env"
...
if [[ -z "$source_config_path" && ( -e "$base_cwd/.agentsai/config.json" || -L "$base_cwd/.agentsai/config.json" ) ]]; then
  source_config_path="$base_cwd/.agentsai/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$agentsai_home/instances/$agentsai_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$agentsai_dir"
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm agentsai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm agentsai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v agentsai >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      agentsai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
async function main() {
  ...
  const agentsaiDir = process.env.PAPERCLIP_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.PAPERCLIP_WORKTREES_DIR) ?? "~/.agentsai-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(agentsaiDir, "config.json");
  const envPath = path.resolve(agentsaiDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "agentsai",
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated AgentsAI worktree config at $worktree_config_path" >&2
else
  if agentsai_command_available; then
    run_isolated_worktree_init
  else
    echo "agentsai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".agentsai-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
