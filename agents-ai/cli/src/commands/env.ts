...
const DEFAULT_AGENT_JWT_ISSUER = "Agents";
const DEFAULT_AGENT_JWT_AUDIENCE = "Agents-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveAgentsInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveAgentsInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" Agents env ")));
...
function collectDeploymentEnvRows(config: AgentsConfig | null, configPath: string): EnvVarRow[] {
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from Agents_PUBLIC_URL when possible)",
    },
    {
      key: "Agents_AGENT_JWT_TTL_SECONDS",
    ...
    {
      key: "Agents_SECRETS_STRICT_MODE",
    ...
    {
      key: "Agents_SECRETS_MASTER_KEY_FILE",
    ...
    {
      key: "Agents_STORAGE_S3_ENDPOINT",
    ...
    {
      key: "Agents_STORAGE_S3_FORCE_PATH_STYLE",
