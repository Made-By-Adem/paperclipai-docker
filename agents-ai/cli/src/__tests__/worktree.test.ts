...
function buildSourceConfig(): AgentsConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("Agents-pr-432")).toBe(
      path.resolve(os.homedir(), "Agents-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("Agents/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://Agents.example", 3110)).toBe("https://Agents.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/Agents-feature",
      homeDir: "/tmp/Agents-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/Agents-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/Agents-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.Agents_HOME).toBe(path.resolve("/tmp/Agents-worktrees"));
    expect(env.Agents_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.Agents_IN_WORKTREE).toBe("true");
    expect(env.Agents_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.Agents_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export Agents_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("Agents-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-secrets-"));
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          Agents_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-jwt-"));
    ...
    try {
      ...
      await worktreeInitCommand({
        ...
        home: path.join(tempRoot, ".Agents-worktrees"),
      });

      const envPath = path.join(repoRoot, ".Agents", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("Agents_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("Agents_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/Agents_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".Agents-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("Agents-worktree-auth-source-");

      try {
        const sourceDbClient = createDb(sourceDb.connectionString);
        await sourceDbClient.insert(authUsers).values({
          id: "user-existing",
          email: "existing@Agents.ing",
          name: "Existing User",
          emailVerified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".Agents", "config.json"), "utf8"),
        ) as AgentsConfig;
        ...
        try {
          const targetDb = createDb(
            `postgres://Agents:Agents@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/Agents`,
          );
          const seededUsers = await targetDb.select().from(authUsers);
          expect(seededUsers.some((row) => row.email === "existing@Agents.ing")).toBe(true);
        } finally {
          await targetPg.stop();
        }
      } finally {
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".Agents-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".Agents", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Agents config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".Agents", "config.json");
    const originalCwd = process.cwd();
    const originalAgentsConfig = process.env.PAPERCLIP_CONFIG;
    ...
    try {
      ...
      if (originalAgentsConfig === undefined) {
        ...
        process.env.PAPERCLIP_CONFIG = originalAgentsConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".Agents", "config.json");
    const envPath = path.join(worktreeRoot, ".Agents", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "Agents_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".Agents", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".Agents", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Agents instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".Agents-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalAgentsConfig = process.env.PAPERCLIP_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `Agents_HOME=${homeDir}`,
          `Agents_INSTANCE_ID=${currentInstanceId}`,
          "Agents_WORKTREE_NAME=existing-name",
          "Agents_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`Agents_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("Agents_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("Agents_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalAgentsConfig === undefined) {
        delete process.env.PAPERCLIP_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".Agents-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `Agents_HOME=${homeDir}\nAgents_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`Agents_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/Agents",
        targetRepoRoot: "/Users/example/Agents-pr-432",
        workspaceCwd: "/Users/example/Agents",
      }),
    ).toBe("/Users/example/Agents-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/Agents",
        targetRepoRoot: "/Users/example/Agents-pr-432",
        workspaceCwd: "/Users/example/Agents/packages/db",
      }),
    ).toBe("/Users/example/Agents-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "Agents-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("Agents-make-test", {
        seed: false,
        home: path.join(tempRoot, ".Agents-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".Agents", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".Agents", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".Agents", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".Agents", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Agents metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".Agents", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".Agents-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".Agents-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".Agents", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".Agents", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".Agents", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("Agents-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
