import { asBoolean, asString, asStringArray } from "@agentsai/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Agents will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
