...
const DRAFT_KEY = "Agents:issue-draft";
...
function defaultExecutionWorkspaceModeForProject(
  ...
  const defaultMode = project?.executionWorkspacePolicy?.enabled
...
function issueExecutionWorkspaceModeForExistingWorkspace(
...
export function NewIssueDialog() {
  ...
  useEffect(() => {
    ...
    if (newIssueDefaults.parentId) {
      ...
      const defaultExecutionWorkspaceMode =
        newIssueDefaults.executionWorkspaceId
          ? "reuse_existing"
          : (newIssueDefaults.executionWorkspaceMode ??
            defaultExecutionWorkspaceModeForProject(defaultProject));
      setIssueText(
  ...
  const deduplicatedReusableWorkspaces = useMemo(() => {
    ...
    for (const ws of workspaces) {
      ...
      if (
        !existing ||
        new Date(ws.lastUsedAt) > new Date(existing.lastUsedAt)
      ) {
        seen.set(key, ws);
      }
    }
            ...
            <div className="px-4 py-3 space-y-2">
              <div className="space-y-1.5">
                ...
                <div className="text-[11px] text-muted-foreground">
                  Control whether this issue runs in the shared workspace, a new
                  isolated workspace, or an existing one.
                </div>
                <select
                ...
                {executionWorkspaceMode === "reuse_existing" && (
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedExecutionWorkspaceId(e.target.value)
                    }
                  >
                ...
                {showParentWorkspaceWarning ? (
                  <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                    Warning: this sub-issue will no longer use the parent issue
                    workspace
