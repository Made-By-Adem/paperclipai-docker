 ...
 * toggle server-side adapter code for the whole Agents instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-Agents-adapter") or local path */
  packageName: string;
...
function buildAdapterCapabilities(
  ...
  return {
    ...
    requiresMaterializedRuntimeSkills:
      adapter.requiresMaterializedRuntimeSkills ?? false,
  };
}
...
export function adapterRoutes() {
  ...
  // without modifying Agents's source code.
  //
