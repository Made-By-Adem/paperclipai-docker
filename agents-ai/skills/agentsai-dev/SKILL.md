...
required: false
description: >
  Develop and operate a local AgentsAI instance — start and stop servers,
  ...
  AgentsAI codebase itself or keep a running instance healthy.
---

# AgentsAI Dev

This skill covers the day-to-day workflows for developing and operating a local AgentsAI instance. It assumes you are working inside the AgentsAI repo checkout with `origin` pointing to `git@github.com:agentsai/agentsai.git`.
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the AgentsAI repo. It is the canonical reference for all `agentsai` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx agentsai run` |
...
| Back up database | `npx agentsai db:backup` |
| Health check | `npx agentsai doctor --repair` |
| Print env vars | `npx agentsai env` |
| Trigger agent heartbeat | `npx agentsai heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx agentsai agent local-cli <agent> --company-id <id>` |
...
AgentsAI worktrees combine git worktrees with isolated AgentsAI instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own AgentsAI environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing AgentsAI changes in isolation before merging
...
| `worktree:list` | List worktrees and their AgentsAI status |
...
npx agentsai worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx agentsai worktree env)"

# 3. Start the isolated AgentsAI server
npx agentsai run

# 4. Do your work

# 5. When done, merge history back if needed
npx agentsai worktree:merge-history --from agentsai-my-feature --to current --apply

# 6. Clean up
npx agentsai worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `agentsai/agentsai` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`agentsai` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/agentsai` (or `github.com/<user>/agentsai.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `agentsai/agentsai` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo agentsai/agentsai --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `agentsai/agentsai` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `agentsai/agentsai` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/agentsai\/agentsai.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx agentsai` / `pnpm agentsai` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.agentsai/`, `.agentsai-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `agentsai` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx agentsai doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx agentsai worktree env)"
   pnpm install && pnpm build
   npx agentsai run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx agentsai worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx agentsai worktree:cleanup <name>
   npx agentsai worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx agentsai worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx agentsai doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx agentsai worktree env)"` after cd-ing into the worktree |
...
| Pushed feature branch to `agentsai/agentsai` when a fork exists | Push to the user's fork remote instead — see "Forks" above |
