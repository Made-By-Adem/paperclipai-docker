...
This document defines the V1 design for a AgentsAI adapter that integrates with
...
4. synthesized stdout events for AgentsAI UI/CLI
...
3. **Agent callback auth to AgentsAI** uses a bootstrap exchange flow (no long-lived AgentsAI key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from AgentsAI endpoints, not full SKILL.md prompt injection.
...
- `agentsaiPublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
## AgentsAI Callback + Auth Flow (V1)
...
1. `adapterConfig.agentsaiPublicUrl`
...
Goal: avoid putting long-lived AgentsAI credentials in prompt text.

Flow:

1. Before launch/follow-up, AgentsAI mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. AgentsAI validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all AgentsAI API calls.
...
1. Prompt includes a compact instruction to fetch skills from AgentsAI.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/agentsai`
   - `GET /api/skills/agentsai-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <agentsaiPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active AgentsAI run context.
...
5. `agentsaiPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from AgentsAI skill endpoints |
| AgentsAI auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from AgentsAI context.
3. Add richer PR metadata surfacing in AgentsAI UI.
