...
Today release smoke testing for published AgentsAI packages is manual and shell-driven:
...
- runs `agentsai@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@agentsai.local`
- password: `agentsai-smoke-password`
...
1. A published `agentsai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `agentsai@latest` Docker install can be smoke-tested by Playwright in CI.
