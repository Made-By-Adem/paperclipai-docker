...
function resolveAgentJwtSecretStatus(
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.Agents_AGENT_JWT_SECRET === "string" ? parsed.Agents_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm Agentsai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveAgentsEnvPath();
