...
Example AgentsAI plugin that demonstrates:
...
```bash
pnpm --filter @agentsai/plugin-file-browser-example build
pnpm agentsai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm agentsai plugin uninstall agentsai-file-browser-example --force
```
...
- Optional: use `agentsai-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
